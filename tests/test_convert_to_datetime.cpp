...
class ConvertToDateTimeTest : public ::testing::Test {
  protected:
    static std::tm createTimeStruct(int year, int month, int day, int hour,
...
TEST_F(ConvertToDateTimeTest, ValidFullDatetime) {
    blpapi::Datetime blpDatetime(2023, 10, 15, 14, 30, 45,
                                 blpapi::Datetime::Microseconds(123456));
    ...
    std::time_t expectedTimeSinceEpoch = timegm(&timeStruct);
    uint64_t expectedMicros =
        static_cast<uint64_t>(expectedTimeSinceEpoch) * 1000000 + 123456;
...
TEST_F(ConvertToDateTimeTest, ValidMinimalDatetime) {
    blpapi::Datetime blpDatetime(2023, 1, 1, 0, 0, 0,
                                 blpapi::Datetime::Microseconds(0));
    ...
    std::time_t expectedTimeSinceEpoch = timegm(&timeStruct);
    uint64_t expectedMicros =
        static_cast<uint64_t>(expectedTimeSinceEpoch) * 1000000;
    EXPECT_EQ(dateTime.microseconds, expectedMicros);
    EXPECT_EQ(dateTime.offset, 0);
}

TEST_F(ConvertToDateTimeTest, InvalidUnsetYear) {
    blpapi::Datetime blpDatetime(0, 10, 15, 14, 30, 45,
                                 blpapi::Datetime::Microseconds(123456));
...
TEST_F(ConvertToDateTimeTest, InvalidMonthOutOfRange) {
    blpapi::Datetime blpDatetime(
        2023, 13, 15, 14, 30, 45,
        blpapi::Datetime::Microseconds(123456)); // Invalid month
    blpDatetime.setOffset(120);
...
TEST_F(ConvertToDateTimeTest, InvalidDayOutOfRange) {
    blpapi::Datetime blpDatetime(
        2023, 2, 30, 14, 30, 45,
        blpapi::Datetime::Microseconds(123456)); // Invalid day
    blpDatetime.setOffset(120);
